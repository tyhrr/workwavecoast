...
def get_database() -> Optional[Database]:
    """Get database instance using global manager
    ...
    try:
        ...
        if db_manager.db_config is None:
            mongo_uri = os.getenv('MONGODB_URI', 'mongodb://localhost:27017/workwave_coast')
            db_manager.initialize(mongo_uri)

        if db_manager.db_config:
            return db_manager.db_config.db
        else:
            return None
    except Exception as e:
